...
func Validate(fs sriracha.FieldSet) error {
	if fs.Version == "" {
		return fmt.Errorf("fieldset: version must not be empty")
	}
...
func NegotiateVersion(supported []string, requested string) (string, error) {
	if len(supported) == 0 {
		return "", fmt.Errorf("fieldset: no supported versions")
	}
	if requested == "" {
		return "", fmt.Errorf("fieldset: requested version is empty")
	}
	best := ""
...
// Callers are responsible for validating version format if strictness is needed.
func parseSemver(v string) [3]int {
	parts := strings.SplitN(v, ".", 3)
	var result [3]int
	for i := 0; i < 3 && i < len(parts); i++ {
		result[i] = parseIntSafe(parts[i])
	}
	return result
}
